...
void RestState::Load() {
	...
	healbutton.RS.gfx.BM = AE_GFX_BM_BLEND;
...
void RestState::Update(f64 deltaTime) {
	...
	switch (MODE) {
		...
		case UPGRADEchoice:
			...
			break;
		case UPGRADE:
			timepassed += (float)AEFrameRateControllerGetFrameTime();
...
void RestState::Draw() {
	...
	switch (MODE) {
		case ROOM:
			...
			if (noitems) {
				utils::UDrawText(FontID, "NO ITEMS TO UPGRADE", upgradebutton.RS.t.x , upgradebutton.RS.t.y - upgradebutton.RS.t.h, 1, Color{ 255,255,255 });
			}
			break;
		case HEALING:
			m_context->Player->DrawHPBar(m_context->render, 50.f, 250.f);
		...
		case UPGRADEchoice:
			for (int i = 0; i < 4; i++) {
				...
				if (m_context->Items->items.at(i).level == 0) {
					...
					UDrawText(FontID, "Level " + std::to_string(m_context->Items->items.at(i).level), upgradechoices.at(i).RS.t.x, upgradechoices.at(i).RS.t.y - upgradechoices.at(i).RS.t.h / 2.f-50.f, 1, AM::Color{ 0,0,0});
				}
		...
		case UPGRADE:
			...
			utils::UDrawText(FontID, std::to_string((int)(upgradenum * 100)) + "%", upgradebar.currrender.t.x + upgradebar.currrender.t.w/2.f, upgradebar.fullrender.t.y+upgradebar.fullrender.t.h, 1, Color{ 255,255,255 });
			//5s to get ready
			if (timepassed < 5) {
